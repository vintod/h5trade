define(function(require, exports, module) {
	...
	var szflag = [{id:'0S', name:'限价委托'},{id:'0f', name:'对方最优价格'},{id:'0g', name:'本方最优价格'},{id:'0h', name:'即时成交剩余撤销'},{id:'0i', name:'五档即成剩撤'},{id:'0j', name:'全额成交或撤销'}];
	var shflag = [{id:'0S', name:'限价委托'},{id:'0i', name:'五档即成剩撤'},{id:'0r', name:'五档即成转限价'}];
	...
	function bindPageEvent(){
		...
		appUtils.bindEvent($('.tab li'),function() {
			var url = location.href.replace('entrust_sell', $(this).attr('data-url')).replace(/stkcode=[^&]*/g, '')
		...
		appUtils.bindEvent($('#selRate .pos'),function() {
			$('#qty').val(calcNum($(this).attr('data-rate'))).flash();
			showAmount();
		});
		...
		appUtils.bindEvent($('#btnSubmit'),function() {
			...
			var order = {
				...
				price	: (stkInfo.bsflag=='0S'?$.format($('#price').val(), 2):'1.00'),	//委托价格
	...
	//计算最大可卖数量
	function calcNum(rate) {

		if(parseInt(rate) == 1)
		{
			return !isNaN(stkInfo.stkmaxqty)?parseInt(stkInfo.stkmaxqty):0;
		}
		else
		{
			return !isNaN(stkInfo.stkmaxqty)?parseInt(stkInfo.stkmaxqty*rate*0.01)*100:0;
		}
	...
	function showAmount() {
		...
		$('#qty').attr('placeholder', '可卖：'+calcNum(1));
	}
	...
	function validateOrder(order) {
		...
		if(order.bsflag == '0S' && (parseFloat(order.price) > parseFloat(stkInfo.upprice) || parseFloat(order.price) < parseFloat(stkInfo.downprice))) {
			layerUtils.iMsg('委托价格(' + order.price + ')须在涨停价和跌停价[' + stkInfo.downprice + ',' + stkInfo.upprice + ']之间');
			return false;
		}
		
		var maxNum = calcNum(1);//可卖数量
		if(parseInt(order.qty) <= 0 || parseInt(order.qty) > parseInt(maxNum)) {
			layerUtils.iMsg('委托数量(' + order.qty + ')必须大于0并小于可卖数量(' + maxNum + ')');
			return false;
		}
	...
	function confirmOrder(order) {
		var content = '<p style="text-align:left">股东账号：' + order.secuid + '</p>'
				    ...
				    + (order.bsflag=='0S'?'<p style="text-align:left">委托价格：<span class="red bold">' + order.price + '</span></p>':'')
