define(function(require, exports, module) {
		...
		service.queryBankList(function(data) {
			if(data.error_no == 0) {
				if(data.results && data.results[0]) {
					...
					//查询余额
					queryFundBalance(function(){
						showFundBalance(bankAccts[currCardNo].moneyTypes[0]);//显示余额
					});
	...
	//查询资金账户余额
	function queryFundBalance(callback){
		service.queryDesirableAmount(function(data) {
			if(data.error_no == 0){
				data.results.forEach(function(item){
					balance[item.moneytype] = item.maxdraw;
					$("#funcid").html(item.fundid);
				});
				callback && callback();
			}else{
				layerUtils.iMsg(data.error_info);
	...
	//显示账户余额
	function showFundBalance(moneytype) {
		$("#desirableAmount").html(balance[moneytype]);
	}
	...
	function bindPageEvent(){
		...
		appUtils.bindEvent($('.tab li'),function() {
			appUtils.replaceURL(location.href.replace('transfer_sec2bank', $(this).attr('data-url')));
		});
		
		//选择银行卡
		appUtils.bindEvent($('#btnSelBank'),function() {
			appUtils.replaceURL('./sel_bank.html?url='+escape('./transfer_sec2bank.html') + '&initCardNo=' + currCardNo);
		});
		
		//选择币种
		appUtils.bindEvent($('#selCurrency'),function(){
			$(this).prev().html($(this).find("option:selected").text());
			showFundBalance($(this).val());
		},'change');
	...
	function submit() {
		...
		var str = "<span class='boxCont'>请确认以下转账信息：<br>" 
				+ "银行：" + bankAccts[currCardNo].bankName + "<br>" 
 				+ "币种：" + moneyTypeDict[$('#selCurrency').val()] + "<br>" 
 				+ "金额：" + $("#tranamt").val() + "元<br>" 
 				+ "</span>";
		layerUtils.iAlert(str, null, function(){
			service.queryBankInfo(function(data){
				if(data.error_no == 0){
					if(data.results && data.results.length>0){
						if(data.results[0].fundpwdflag != '0') {//需要输入密码
							showPwdDiv();
						}else{//直接提交
							doBankTrans();
						}
			...
			},bankAccts[currCardNo].bankCode,$('#selCurrency').val(),'2',true);
		}, '立即转出', function(){
	...
	function doBankTrans(){
		...
		}, $('#selCurrency').val(), $("#inputPwd").val(), bankAccts[currCardNo].bankCode, $("#inputPwd").val(), '2', $("#tranamt").val());
	}
